"""
Standalone metrics HTTP server for the Smart Kiosk Assistant.
...
class MetricsHandler(BaseHTTPRequestHandler):
    """Minimal HTTP handler – no external dependencies beyond the stdlib."""
    ...
    def do_GET(self):  # noqa: N802
        ...
        elif path == "/platform-info":
            payload = get_platform_info()
            status  = 200

        elif path == "/memory":
            data    = parse_memory_usage()
            payload = data if data is not None else {"error": "no memory data"}
...
def run_server(host: str = "0.0.0.0", port: Optional[int] = None) -> None:
    ...
    print(
        f"[metrics-collector] listening on {host}:{effective_port}  "
